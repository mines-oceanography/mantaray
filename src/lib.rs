...
impl WaveRayPath {
   pub fn new(gravity: f64, depth_data: Box<dyn Depth>) -> Self {
...
mod test_constant_cg {
   ...
   use crate::{group_velocity, odes, ConstantDepth, ArrayDepth, output_to_file};
   ...
   fn test_array_as_parameter() {
      ...
      for (kx, ky, xf, yf) in check_axis {
         let system = WaveRayPath::new(9.8, Box::new(ArrayDepth { array: vec![
            vec![1000.0, 1000.0],
            vec![1000.0, 1000.0]
         ] }));
