...
// FIXME: there is probably a better looking way to do this
struct FunctionHeight {
   f: fn(f64, f64) -> f64,
}

impl FunctionHeight {
    fn value(&self, f: fn(f64, f64) -> f64, x: f64, y: f64) -> f64 {
      f(x, y)
    }
}

impl Height for FunctionHeight {
   fn calculate(&self, x: f64, y: f64) -> f64 {
       self.value(self.f, x, y)
   }
}

// define a function to test:
fn example_fn(x: f64, y: f64) -> f64 {
   1000.0
}
...
mod test_constant_cg {
   ...
   use crate::{group_velocity, odes, ConstantHeight, FunctionHeight, example_fn, ArrayHeight};
   ...
   #[test]
   // almost identical to the above test_axis, but using FunctionHeight instead of ConstantHeight
   fn test_function_as_parameter() {
      ...
      for (kx, ky, xf, yf) in check_axis {
         let system = WaveRayPath::new(9.8, Box::new(FunctionHeight { f: example_fn }));
         let y0 = State::new(0.0, 0.0, kx, ky);
         let mut stepper = Rk4::new(system, 0.0, y0, 1.0, 1.0);
         if stepper.integrate().is_ok() {
            let last_state = stepper.y_out().last().unwrap();
